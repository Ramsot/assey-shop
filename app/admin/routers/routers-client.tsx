...
export function RoutersClient({ initialRouters }: RoutersClientProps) {
  ...
  const handleSyncStatus = async (id: string) => {
    alert(`Syncing status for router ${id}... (Feature in development)`);
  };

  const handleAddRouter = () => {
    alert("Add Router... (Feature in development)");
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {routers.length > 0 ? (
          ...
          <div className="col-span-full py-20 text-center bg-white/5 border border-white/10 rounded-3xl">
            <RouterIcon className="w-12 h-12 mx-auto mb-4 opacity-10" />
