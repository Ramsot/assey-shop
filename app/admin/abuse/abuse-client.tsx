...
import { clearAbuseLogs, resolveAbuseReport } from "@/modules/admin/admin.actions";

interface AbuseClientProps {
  initialReports: any[];
}

export function AbuseClient({ initialReports }: AbuseClientProps) {
  ...
  const currentLoadingId = actionId;
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button 
          ...
          disabled={loading && currentLoadingId === "clear"}
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 transition-all text-sm font-bold disabled:opacity-50"
        >
          {loading && currentLoadingId === "clear" ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
          Clear All Logs
        </button>
      </div>
                    ...
                    <td className="px-6 py-4 text-right">
                      {report.isResolved ? (
                        ...
                        <button 
                          ...
                          disabled={loading && currentLoadingId === report.id}
                          className="text-xs font-bold text-blue-400 hover:text-blue-300 transition-colors underline underline-offset-4 disabled:opacity-50"
                        >
                          {loading && currentLoadingId === report.id ? "Resolving..." : "Mark Resolved"}
                        </button>
                      )}
                    </td>
                  </tr>
