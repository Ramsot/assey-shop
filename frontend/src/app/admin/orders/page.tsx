...
export default function OrdersPage() {
  ...
  useEffect(() => { fetchOrders(); }, [page, status, dateFrom, dateTo]);
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {(dateFrom || dateTo || status || search) && (
          <button onClick={() => { setSearch(""); setStatus(""); setDateFrom(""); setDateTo(""); setPage(1); }}
            className="rounded-xl border border-border px-4 py-2.5 text-sm text-muted-foreground hover:text-ink hover:bg-accent transition-colors">
            Clear
          </button>
        )}
      </div>
                ...
                orders.map((order, i) => (
                  <motion.tr key={order.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                    ...
                    <td className="px-4 py-3 text-right">
                      <button aria-label="View order" onClick={(e) => { e.stopPropagation(); router.push(`/admin/orders/${order.id}`); }}
